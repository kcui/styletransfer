...
def build_model(img):
  ...
  _, h, w, d = img.shape
  content_image = tf.keras.backend.variable(np.zeros((1, h, w, d), dtype=np.float32))
  # Load our model. We load pretrained VGG, trained on imagenet data
  model = tf.keras.applications.vgg19.VGG19(include_top=False, input_tensor=content_image, weights='imagenet')
  net = dict([(layer.name, layer.output) for layer in model.layers])
  return net
...
def sum_style_losses(sess, net, style_imgs):
  ...
  for img, img_weight in zip(style_imgs, weights):
    sess.run(net['input_1'].assign(img))
    style_loss = 0.
...
def sum_content_losses(sess, net, content_img):
  sess.run(net['input_1'].assign(content_img))
  content_loss = 0.
...
def sum_longterm_temporal_losses(sess, net, frame, input_img):
  x = sess.run(net['input_1'].assign(input_img))
  loss = 0.
